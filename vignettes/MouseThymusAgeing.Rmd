...
The `r Biocpkg("MouseThymusAgeing")` package provides convenient access to the single-cell RNA sequencing (scRNA-seq) datasets from @baran-gale_thymus_2020. The study using single-cell transcriptomic profiling to resolve how the epithelial composition of the mouse thymus 
...
are provided post-processing and QC. The raw sequencing data can be directly acquired from ArrayExpression using accession 
...
Raw sequencing data can be acquired from ArrayExpress accessions [E-MTAB-8560](https://www.ebi.ac.uk/arrayexpress/experiments/E-MTAB-8560/) and 
[E-MTAB-8737](https://www.ebi.ac.uk/arrayexpress/experiments/E-MTAB-8737/).
...
Detailed experimental protocols are available in [the manuscript](10.7554/eLife.56221) and analytical details are provided in the 
accompanying [GitHub repo](https://github.com/WTSA-Homunculus/Ageing2019).
...
and days block sorted into different areas of each plate to minimise the confounding between batch effects and mouse age and sorted subpopulation. The single-cell libraries were prepared according to the [SMART-seq2 protocol]() and sequenced on an Illumina NovaSeq 6000.
...
* Gene counts were normalized using size factors estimated using `computeSumFactors()` function from `r Biocpkg("scran")` [@lun2016pooling].
...
detection algorithm [@pons2005walktrap]. Clusters were manually annotated based on inspecting the expression of marker genes.

The second data contains cells that were profiling from TEC at 4, 16 and 32 weeks old, derived from a transgenic model system that is also able 
...
first had their transgene induced using doxycycline, and 4 weeks later the TEC were collected in separate ZsG+ and ZsG- groups. Within each 
of these geoups cells were FAC-sorted into mTEC (Cd45+EpCam+MHCII+Ly51-UEA1+) and cTEC (Cd45+EpCam+Ly51+UEA1+) populations. For this experiment 
we made us of recent developments in multiplexing with hashtag oligos (HTO; cell-hashing). Consequently, the cells were super-loaded onto the 
10X Genomics Chromium chips before library prep and sequencing on an Illumina NovaSeq 6000.
...
* Non-empty droplets were called separately using the `emptyDrops()` from the `r Biocpkg("DropletUtils")` [@lun2019emptydrops].
...
[@stoechkius2018hashing] - this was also able to identify multiplet cells containing multiple HTOs.
...
* Size factors were estimated as above using `computeSumFactors()` function from `r Biocpkg("scran")` [@lun2016pooling], and used for 
...
[@pons2005walktrap]. These clusters were annotated with concordant labels from the above data set. The exception being that many more clusters 
were identified, and thus each cluster was suffixed with a number to uniquely identify them.

# Package data format

The SMART-seq2 data is stored in subset according to the sorting day (numbered 1-5). For the droplet data, the data can be accessed according 
...
```{r}
head(SMARTseqMetadata, n = 3)
```

All of data access functions allow you to select the particular samples or sorting days that you would like to access for the relevant data 
